...
class BudgetAwarePlannerAgent(BaseAgent):
    ...
    def __init__(self, name: str):
        ...
        budget_parser_agent = LlmAgent(
            name="BudgetParserAgent", model=LiteLlm(model="openai/llama3.2:latest"),
        ...
        activity_finder_agent = LlmAgent(
            name="ActivityFinderAgent", model=LiteLlm(model="openai/llama3.2:latest"), tools=[],
        ...
        cost_estimator_agent = LlmAgent(
            name="CostEstimatorAgent", model=LiteLlm(model="openai/llama3.2:latest"), tools=[],
        ...
        restaurant_finder_agent = LlmAgent(
            name="RestaurantFinderAgent", model=LiteLlm(model="openai/llama3.2:latest"), tools=[],
    ...
    async def _run_async_impl(
        ...
        if total_budget <= 0:
            failure_agent = LlmAgent(name="FailureAgent", model=LiteLlm(model="openai/llama3.2:latest"), instruction="Politely inform the user that a valid budget is needed to begin planning and end the conversation.")
        ...
        if (running_cost + activity_cost) <= total_budget:
            ...
            feedback_agent = LlmAgent(name="FeedbackAgent", model=LiteLlm(model="openai/llama3.2:latest"), instruction=f"Inform the user that '{activity_name}' has been added to the plan. The current total is ${running_cost:.2f} of the ${total_budget:.2f} budget.")
        ...
        **Total Estimated Cost: ${running_cost:.2f}**
        """
        summary_agent = LlmAgent(name="SummaryAgent", model=LiteLlm(model="openai/llama3.2:latest"), instruction=summary_instruction, output_key="final_response")
